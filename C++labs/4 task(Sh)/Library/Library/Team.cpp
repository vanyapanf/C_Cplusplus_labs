...
Vector<Game*>::iterator Team::FindGame1(std::string dateGame) {
	Vector<Game*>::iterator it;
	for (it = GameTab.begin(); it != GameTab.end(); ++it) {
		std::string Date = (**it).GetDate();
		if (Date == dateGame)
...
int Team::DeleteGame(std::string dateGame) {
	Vector<Game*>::iterator G= FindGame1(dateGame);
	delete(*G);
	GameTab.erase(G);
	return 1;
}
int Team::DeletePlayerTeam(int nom) {
	Vector<Player *>::iterator P = FindPlayerTeam1(nom);	
	delete(*P);
	PlayerTab.erase(P);
	return 1;
}
...
Vector<Player*>::iterator Team::FindPlayerTeam1(int nom) {
    Vector<Player*>::iterator it;
	for (it = PlayerTab.begin(); it != PlayerTab.end(); ++it) {
		int nomer = (**it).GetNom();
		if (nomer == nom)
