...
int DFind(Table *tbl) {
	...
	if (fl == 0)
	{
		...
		while ((i = Find(tbl, key, release)) >= 0) {
            ...
            (&tbl1)->tbl[(&tbl1)->n].info = tbl->tbl[i].info;*/
...
/*int Add(Table *tbl, int key, char *info) {
	...
	tbl->tbl [tbl->n].key = key;
	tbl->tbl[tbl->n].release = release;
	tbl->tbl[tbl->n].info = _strdup(info);
	tbl->n++;
	return 0;
}*/
int DDelete(Table *tbl) {
	...
	if (tbl->n == 0)
		...
		tbl->n = tbl1->n;
		for (i = 0; i < tbl1->n; ++i) {
			tbl->tbl[i].key = tbl1->tbl[i].key;
			tbl->tbl[i].release = tbl1->tbl[i].release;
			tbl->tbl[i].offset = tbl1->tbl[i].offset;
			tbl->tbl[i].len = tbl1->tbl[i].len;
		}
		printf("OK\n");
	}
	return 0;
